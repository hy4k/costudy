...
export const LibraryVault: React.FC = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto px-12 py-20 relative">
            ...
            {showSearchResults && (
                <div className="fixed inset-0 z-40 bg-slate-900/60 backdrop-blur-2xl border border-white/10 flex items-start justify-center p-10 pt-20 animate-in fade-in zoom-in-95 duration-500 overflow-y-auto">
                    <div className="bg-white/5 border border-white/10 p-12 rounded-xl max-w-5xl w-full relative shadow-[0_0_100px_rgba(255,26,26,0.1)]">
                        <button onClick={() => setShowSearchResults(false)} className="absolute top-8 right-8 p-4 hover:bg-white/10 rounded-lg transition-all text-white px-4 py-2">
                        ...
                        {isSearching ? (
                            ...
                            <div className="space-y-6">
                                {searchResults.map((hit, i) => (
                                    <div key={i} className="bg-white/[0.03] border border-white/5 p-8 rounded-xl hover:border-brand/30 transition-all">
            ...
            {showArchitecture && (
                <div className="fixed inset-0 z-30 bg-slate-900/60 backdrop-blur-2xl border border-white/10 flex items-center justify-center p-10 animate-in fade-in zoom-in-95 duration-500">
                    <div className="bg-white/5 border border-white/10 p-16 rounded-xl max-w-5xl w-full max-h-[85vh] overflow-y-auto no-scrollbar relative shadow-[0_0_100px_rgba(255,26,26,0.1)]">
                        <button onClick={() => setShowArchitecture(false)} className="absolute top-10 right-10 p-4 hover:bg-white/10 rounded-lg transition-all text-white px-4 py-2">
                            <Icons.Plus className="w-8 h-8 rotate-45" />
                        </button>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
                            <div className="p-8 bg-white/[0.02] border border-white/5 rounded-xl space-y-6">
                                ...
                                <div className="bg-black/40 p-6 rounded-xl font-mono text-[10px] text-brand-300">
                            ...
                            <div className="p-8 bg-white/[0.02] border border-white/5 rounded-xl space-y-6">
                                ...
                                <div className="bg-black/40 p-6 rounded-xl font-mono text-[10px] text-emerald-400 overflow-x-auto">
                                    {`CREATE INDEX ON vault_vectors \nUSING hnsw (embedding vector_cosine_ops);`}
                                </div>
                            </div>
                        </div>

                        <div className="bg-brand/10 border border-brand/20 p-10 rounded-xl text-center">
            ...
            <header className="mb-20 text-center relative">
                ...
                <div className="flex justify-center gap-4 mb-8">
                    <div className="inline-flex items-center gap-3 bg-slate-900 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.4em] shadow-2xl">
                    ...
                    <button 
                        ...
                        className="inline-flex items-center gap-3 bg-white border border-slate-200 text-slate-900 px-6 py-3 rounded-lg text-[10px] font-black uppercase tracking-[0.4em] hover:bg-brand hover:text-white transition-all shadow-xl"
                    >
            ...
            <div className="flex flex-col md:flex-row gap-12 mb-16 items-center justify-between bg-white/40 backdrop-blur-xl p-8 rounded-xl border border-white/80 shadow-sm">
                <div className="relative w-full max-w-xl">
                    <input 
                        ...
                        className="w-full bg-white border border-slate-300 px-10 py-5 rounded-lg text-sm font-bold text-slate-900 shadow-sm outline-none pl-16 pr-32 transition-all focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    />
                    ...
                    {apiStatus === 'online' && (
                        <button 
                            ...
                            className="absolute right-3 top-1/2 -translate-y-1/2 px-6 py-3 bg-brand text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            RAG Search
                        </button>
                    )}
                ...
                <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                    {['All Content', 'Financial Accounting', 'Strategy', 'Practice'].map(cat => (
                        <button 
                            ...
                            className={`px-8 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${selectedCategory === cat ? 'bg-slate-900 text-white shadow-xl' : 'bg-white text-slate-400 border border-slate-100 hover:border-brand/30'}`}
                        >
                            {cat}
                        </button>
                    ))}
            ...
            {loading ? (
                <div className="flex flex-col items-center py-32 gap-6 text-slate-300 px-4 transition-all">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                    {filtered.map(item => (
                        <div key={item.id} className="group bg-white/80 backdrop-blur-3xl border border-white/80 p-10 rounded-xl shadow-xl hover:shadow-2xl transition-all duration-500 relative overflow-hidden flex flex-col min-h-[420px]">
                            {processingId === item.id && (
                                <div className="absolute inset-0 z-30 bg-slate-950/98 backdrop-blur-md p-10 flex flex-col justify-center animate-in fade-in duration-300">
                                    ...
                                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 bg-black/30 p-6 rounded-xl border border-white/5">
                            ...
                            <div className="relative z-10 pt-8 border-t border-slate-100/80">
                                ...
                                {item.isIndexed ? (
                                    <div className="w-full py-5 bg-emerald-50 text-emerald-600 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 border border-emerald-100 shadow-sm shadow-emerald-500/5 transition-all">
                                    ...
                                    <button 
                                        ...
                                        className="w-full py-5 bg-slate-900 text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-brand transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3 px-4"
                                    >
