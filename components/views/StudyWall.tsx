...
import { summarizePost, textToSpeech } from '../../services/geminiService';
...
export const StudyWall: React.FC<StudyWallProps> = ({ setView, isLoggedIn = false, userId, onAuthRequired, mode = 'PUBLIC' }) => {
  ...
  // Auto-activate Focus Mode when modal opens
  useEffect(() => {
    if (isPostModalOpen) {
      setIsFocusMode(true);
    }
  ...
  const handleVouch = async (postId: string) => {
      ...
      await costudyService.likePost(postId);
  };
  ...
  return (
    <div className={`max-w-[92%] mx-auto px-4 sm:px-6 py-12 flex flex-col items-center overflow-visible transition-all duration-700 ${isFocusMode ? 'pt-0' : 'py-12'}`}>
      ...
      {isPostModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-2xl flex items-center justify-center p-6 animate-in fade-in duration-500">
          <div className="bg-white rounded-[3rem] sm:rounded-[4rem] w-full max-w-2xl p-8 md:p-12 shadow-2xl relative animate-in zoom-in-95 duration-500 overflow-visible border border-white/50 max-h-[90vh] overflow-y-auto no-scrollbar">
            <button 
              onClick={() => { setIsPostModalOpen(false); setIsFocusMode(false); }}
            ...
            <div className="mb-8 mt-4 sm:mt-0">
                <div className="flex items-center gap-3 mb-2">
                    <div className="p-2 bg-brand/10 text-brand rounded-lg animate-pulse"><Icons.Sparkles className="w-4 h-4" /></div>
                    <span className="text-[10px] font-black text-brand uppercase tracking-[0.4em]">Concentrated Context</span>
                </div>
      ...
      {/* Focus Mode Exit Bar */}
      {isFocusMode && !isPostModalOpen && (
          <div className="fixed top-24 left-1/2 -translate-x-1/2 z-20 animate-in slide-in-from-top-10 duration-500">
              <button 
                onClick={() => setIsFocusMode(false)}
                className="bg-slate-900 text-white px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-[0.4em] shadow-2xl flex items-center gap-3 border border-white/10 hover:bg-brand transition-all"
              >
                  <Icons.Plus className="w-4 h-4 rotate-45" /> Exit Zen Mode
              </button>
          </div>
      )}
      ...
      {!isFocusMode && (
        <header className="w-full text-center mb-16 relative py-12 sm:py-24 px-6 sm:px-10 overflow-hidden rounded-[4rem] sm:rounded-[5.5rem] bg-white/40 backdrop-blur-3xl border border-white/80 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.08)] ring-1 ring-white/50 animate-in fade-in duration-700">
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <div className="mt-12 sm:mt-16 w-full flex flex-col items-center gap-8">
              ...
              <button 
                ...
                className="group flex items-center gap-3 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-brand transition-colors"
              >
                  <div className="p-2 rounded-lg bg-slate-100 group-hover:bg-brand/10 transition-colors">
      ...
      {/* Content Wall */}
      <div className={`w-full mx-auto max-w-6xl transition-all duration-700 ${isFocusMode ? 'mt-10' : ''}`}>
          {loading ? (
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-6 sm:gap-0 pt-8 sm:pt-12 border-t border-slate-100/60">
                    ...
                    <div className="w-full sm:w-auto">
                        <button 
                          ...
                          className={`w-full sm:w-auto px-6 sm:px-10 py-4 sm:py-5 rounded-[2rem] text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] border transition-all shadow-sm ${activeSummaryId === post.id ? 'bg-brand text-white border-brand' : 'bg-brand/5 text-brand border-brand/10 hover:bg-brand hover:text-white'}`}
                        >
                          {isSummarizing && activeSummaryId === post.id ? 'Syncing...' : 'Summary'}
                        </button>
                    </div>
                  </div>

                  {activeSummaryId === post.id && summary && (
                    <div className="mt-10 p-8 bg-slate-50 rounded-[3rem] border border-slate-100 animate-in slide-in-from-top-4 duration-500 relative">
                        <div className="flex justify-between items-center mb-6">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-brand/10 rounded-lg"><Icons.Sparkles className="w-4 h-4 text-brand" /></div>
                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">AI Concentrated Insight</span>
                            </div>
                            <button 
                                onClick={() => handleListen(summary)}
                                disabled={isSpeaking}
                                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${isSpeaking ? 'bg-brand text-white' : 'bg-white text-slate-400 hover:text-brand border border-slate-200 shadow-sm'}`}
                            >
                                {isSpeaking ? <Icons.CloudSync className="w-3.5 h-3.5 animate-spin" /> : <Icons.Volume2 className="w-3.5 h-3.5" />}
