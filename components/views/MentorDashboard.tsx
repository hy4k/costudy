    ...
    useEffect(() => {
        const init = async () => {
            ...
            if (session?.user) {
                ...
                const [studentData, broadcastData, roomData, bountyData] = await Promise.all([
            ...
            <div className="max-w-7xl mx-auto px-6 lg:px-12 py-16 animate-in fade-in zoom-in-95 duration-500 specialist-mode">
                ...
                <div className="bg-white rounded-[4rem] border border-slate-200 shadow-2xl overflow-hidden relative">
                     ...
                     <div className="p-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
                        {/* Performance Stats & Category Mastery Chart */}
                        <div className="lg:col-span-2 space-y-12">
                            <StudentMasteryChart 
                                students={[
                                    { id: selectedStudent.id || 'stu-1', name: selectedStudent.name, handle: selectedStudent.handle }
                                ]}
                                selectedStudentId={selectedStudent.id || 'stu-1'}
                            />

                            <div>
                                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mb-8">Performance Breakdown</h3>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 py-16 animate-in fade-in duration-700 specialist-mode">
            ...
            {activeTab === 'IMPACT' && (
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    {/* Recharts Student Mastery Analytics Chart */}
                    <div className="lg:col-span-4 mt-2">
                        <StudentMasteryChart 
                            students={[
                                { id: 'all', name: 'Cohort Average', handle: 'all_cohort' },
                                ...students.map(s => ({ id: s.id, name: s.name, handle: s.handle }))
                            ]}
                            onSelectStudent={(sId) => {
                                if (sId !== 'all') {
                                    handleStudentClick(sId);
                                }
                            }}
                        />
                    </div>
