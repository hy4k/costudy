...
export const ExamSession: React.FC<ExamSessionProps> = ({ testId, title, questionCount, durationMinutes, onExit, userId }) => {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      // Fetch questions from database (MCQs + Essays combined)
      // The service now handles fetching real essay questions from Supabase
      const allQuestions = await fetchExamQuestions(questionCount, 2);
      
      // Questions are already ordered: MCQs first, then Essays
      const fullExam = allQuestions;
