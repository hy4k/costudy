...
export const ExamSession: React.FC<ExamSessionProps> = ({ testId, title, questionCount, durationMinutes, onExit }) => {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      ...
      // Inject Essay Questions (fetches from Supabase essay_questions table with fallback)
  ...
  const handleFinishTest = () => {
    ...
    // Save essay responses to Supabase essay_responses table
             ...
             <div className="mb-8 w-40">
                <div className="border border-slate-200 p-2">
                   <div className="flex items-center gap-2 text-slate-700">
                      ...
                      <span className="text-[6px] uppercase leading-tight font-bold text-slate-500">IMA's Certification for<br/>Accountants and<br/>Financial Professionals<br/>in Business</span>
                   </div>
                </div>
             </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 flex flex-col items-center bg-white relative">
                    ...
                    <div className="w-full max-w-4xl flex-1 border-[3px] border-[#f7b500] rounded-xl p-1 mb-8 relative bg-white">
                        <div className="h-full max-h-[400px] overflow-y-auto p-8 text-justify text-sm leading-relaxed text-slate-800 pr-6">
                            ...
                            <p className="mb-6">
                                I hereby attest that I will not remove any examination materials from the test center...
                            </p>
  ...
  if (phase === 'TEST') {
    ...
    // Fix: Explicitly type 'a' as Answer to avoid 'unknown' type error in Array.from(Map.values())
                    ...
                    <div className="absolute bottom-0 left-0 w-72 h-[450px] z-40 bg-[#f0f0f0] border-t border-r border-slate-400 shadow-[5px_-5px_20px_rgba(0,0,0,0.2)] flex flex-col animate-in slide-in-from-left duration-200">
                        ...
                        <div className="p-4 bg-[#e6e6e6] flex-1 overflow-y-auto">
                            ...
                            <div className="grid grid-cols-5 gap-1.5">
                                {questions.map((q, idx) => {
                                    // Fix: Explicitly cast the result of Map.get to Answer | undefined to avoid 'unknown' type error
                                    const ans = answers.get(q.id) as Answer | undefined;
                                    return (
                                        <button 
                                            key={q.id}
                                            ...
                                            className={`h-8 border relative font-bold text-xs flex items-center justify-center transition-all ${
                                                ans?.selected ? 'bg-[#8dc63f] text-white border-[#7db536]' : 'bg-white text-slate-700 border-slate-300 hover:bg-slate-100'
                                            }`}
                                        >
                                            {idx + 1}
      ...
      <div className="min-h-screen bg-slate-100 flex items-center justify-center font-sans p-6">
         <div className="bg-white shadow-2xl max-w-2xl w-full border border-slate-300">
            ...
            <div className="p-10 flex flex-col items-center">
               ...
               <p className="text-slate-500 mb-8 font-medium">Your MCQs have been scored. Click below for Gemini AI Essay Evaluation & MCQ Diagnostics.</p>
               ...
               {analysis ? (
                 <div className="w-full mb-8 p-8 bg-slate-900 text-white rounded-3xl text-left max-h-[400px] overflow-y-auto no-scrollbar border-4 border-brand/20 shadow-2xl">
                 ...
                 <button 
                  ...
                  className="w-full mb-8 py-6 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-[0.3em] hover:bg-black transition-all shadow-xl flex items-center justify-center gap-3 disabled:opacity-50"
                 >
                    {isAnalyzing ? (
                      <><Icons.CloudSync className="w-5 h-5 animate-spin" /> Gemini 3.1 Reasoning...</>
                    ) : (
